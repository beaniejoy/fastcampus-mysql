...
- 낙관적 락
  ...
  - CAS(Compare And Set)
  - 레코드 하나에 버전을 부여
  - 다른 Tx에서 버전으로 where 절 조회를 해서 update를 할 때 버전에 해당하는 레코드 없는 경우 실패로 처리된다(예외)
  - 그래서 개발자가 이러한 상황에 대비해 직접 예외처리를 해야 한다

### 좋아요 수 집계 테이블 분리

기존에 Post 테이블에 likeCount 칼럼 추가하는 방식의 문제점
- 어떤 회원이 좋아요를 했는지 알 수 없기 때문에 한 회원이 여러번 좋아요 가능
- 성능상 Post 하나의 데이터에 병목현상이 생기게 됨
  - 하나의 게시글에 1억명의 유저가 좋아요를 누르면 비관적 락이면 무한대기를, 낙관적 락이면 무한실패를 겪게 될 수 있다.

테이블 분리(PostLike)시 장점
- 테이블 분리하여 성능상 이슈 분리
- 어떤 회원이 좋아요했는지 트래킹 가능
- insert만 이루어지기 때문에 locking이 필요 X

<br>

## 이후 고려해볼 내용

- 팔로워가 100만명인 유저 게시물 작성 성능 테스트
  - 비동기 큐를 통해 개선
